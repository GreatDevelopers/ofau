...
def phonetic_search(request):
	"""
	**phonetic_search**
	It's a search module that searches on the bases of similar sounding words,
	ignores vowels, but works fine only if the clients are registred without 
	any spaces in first name/middle name/last name
	"""
	query_string = ''
	found_entries = ''
	if ('q' in request.GET) and request.GET['q'].strip():
		query_string = request.GET['q']
		soundexcode = soundex(query_string)
		compare_soundex = get_query(soundexcode[:5], ['soundex_fname', 'soundex_mname', 
	'soundex_lname', 'soundex_company', 'soundex_address', 'soundex_city'])
		found_entries = Soundexsearch.objects.filter(compare_soundex).values('user__first_name',
               	'user__middle_name','user__last_name', 'user__address', 'user__city', 'user__company', 'user_id')
	temp = {'query_string': query_string, 'found_entries' : found_entries}
	return render_to_response('tcc/search_results.html', dict(temp.items() + tmp.items()),
	context_instance=RequestContext(request)) 


def phono_search(request):
	"""
...
#	return HttpResponse("You're voting on poll")
	temp = {'query_string': query_string, 'found_entries' : found_entries}
	return render_to_response('tcc/phono_search.html', dict(temp.items() + tmp.items()),
	context_instance=RequestContext(request))
