	...
	if form1.is_valid():
	   ...
	   report.save()
		...
		Cement_opc_43.ip_address = request.META['REMOTE_ADDR']
           ...
           #return HttpResponseRedirect('thanks') # Redirect to a 'success' page
	   return HttpResponseRedirect(reverse('ofau.report.views.result_Cement_OPC_43F'))
    else:
        report_form = ReportForm()
...
def result_is_2062_2006(request):
	...
	ID = Id['Report_id__max']
	IS2062 = IS_2062_2006.objects.filter(Report_id = ID)
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
	Head = Report.objects.filter(id = ID)
	...
	organisation = Organisation.objects.all().filter(id = 1)
	department = Department.objects.all().filter(id = 1)
	return render_to_response('report/IS_2062_2006.html', {'IS2062': IS2062, 'Head':Head, 	'organisation':organisation,'department':department,'client':client,},context_instance=RequestContext(request))
