...
def Soil_sample(request):
    ...
    job =Job.objects.get(id=request.GET['id'])
    soil_sample_FormSet = formset_factory(Soil_SampleForm, max_num=30, formset=RequiredFormSet)
    ...
    if request.method == 'POST': # If the form has been submitted...
        ...
        # Create a formset from the submitted data
        soil_sample_formset = soil_sample_FormSet(request.POST, request.FILES)

        if form1.is_valid() and soil_sample_formset.is_valid():
           soil_sample = form1.save(commit=False)
           soil_sample.save()
           for form in soil_sample_formset.forms:
                soil_sample = form.save(commit=False)
                soil_sample.Report_id = report
                soil_sample.ip_address = request.META['REMOTE_ADDR']
                soil_sample.save()

           #return HttpResponseRedirect('thanks') # Redirect to a 'success' page
           return HttpResponseRedirect(reverse('Automation.report.views.result_soil_sample'))
    else:
        report_form = ReportForm()
        soil_sample_formset = soil_sample_FormSet()
...
def result_soil_sample(request):
	Id = Soil_Sample.objects.aggregate(Max('Report_id'))
	ID = Id['Report_id__max']
	st = Soil_Sample.objects.filter(Report_id = ID)
	Id = Report.objects.aggregate(Max('id'))
	ID = Id['id__max']
	Head = Report.objects.filter(id = ID)
	...
	organisation = Organisation.objects.all().filter(id = 1)
	department = Department.objects.all().filter(id = 1)
	return render_to_response('report/soil_sample.html', {'st': st, 'Head':Head, 		'organisation':organisation,'department':department,'client':client,},context_instance=RequestContext(request))
